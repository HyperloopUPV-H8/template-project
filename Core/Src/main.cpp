...
int main(void)
{
  ...
  if (HAL_FMAC_FilterPreload(&hfmac, NULL,         0,
  ...
  HalfOutputDataReadyCallback = CALLBACK_NOT_CALLED;
  OutputDataReadyCallback = CALLBACK_NOT_CALLED;

  uint16_t *size = new uint16_t;
  *size = INPUT_BUFFER_SIZE;

  /*if(HAL_FMAC_ConfigFilterOutputBuffer(&hfmac, aCalculatedFilteredData, &ExpectedCalculatedOutputSize) != HAL_OK){
	  Error_Handler();
  }*/

  if (HAL_FMAC_AppendFilterData(&hfmac, aInputValues, size) != HAL_OK)
  {
    Error_Handler();
  }
